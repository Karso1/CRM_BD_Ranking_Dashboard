...
const people: RankRow[] = [
  { rank: 1, name: "Katrina", owner: "Katrina", type: "总体", target: 1250000, recharge: 2576663, yesterday: 178021, cards: 7187, change: 3 },
  { rank: 2, name: "VICTOR", owner: "VICTOR", type: "总体", target: 1250000, recharge: 1151662, yesterday: 42423, cards: 1544, change: 1 },
  { rank: 3, name: "RUSLAN", owner: "RUSLAN", type: "总体", target: 1250000, recharge: 866915, yesterday: 42309, cards: 2996, change: 2 },
  { rank: 4, name: "Richard", owner: "Richard", type: "总体", target: 1250000, recharge: 338597, yesterday: 6279, cards: 13, change: -1 },
  { rank: 5, name: "Mike", owner: "Mike", type: "总体", target: 1250000, recharge: 128356, yesterday: 2097, cards: 27, change: -2 },
  { rank: 6, name: "Patrick", owner: "Patrick", type: "总体", target: 1250000, recharge: 22830, yesterday: 1813, cards: 9, change: -1 },
  { rank: 7, name: "Others", owner: "Others", type: "总体", target: 3224400, recharge: 1261127, yesterday: 14832, cards: 0, change: 0 },
];
...
export default function Home() {
  ...
  const source =
    view === "总体"
      ? (() => {
          const parentRows = people.filter(
            (item) => owner === "全部BD" || item.owner.toLowerCase() === owner.toLowerCase(),
          );
          if (parentRows.length || owner === "全部BD") return parentRows;

          const childRows = agents.filter((item) => item.owner.toLowerCase() === owner.toLowerCase());
          ...
          return [
            {
              ...
              owner,
              type: "总体" as View,
              target: childRows.reduce((sum, item) => sum + item.target, 0),
              recharge: childRows.reduce((sum, item) => sum + item.recharge, 0),
              yesterday: childRows.reduce((sum, item) => sum + item.yesterday, 0),
              cards: childRows.reduce((sum, item) => sum + item.cards, 0),
              change: 0,
            },
          ];
        })()
      : agents.filter((item) => item.type === view);
    const key = metric === "充值金额" ? "recharge" : metric === "完成率" ? "target" : "yesterday";
    return source.filter((item) => owner === "全部BD" || item.owner.toLowerCase() === owner.toLowerCase()).filter((item) => `${item.name} ${item.owner}`.toLowerCase().includes(search.toLowerCase())).sort((a, b) => metric === "完成率" ? b.recharge / b.target - a.recharge / a.target : b[key] - a[key]);
  ...
  const overview = useMemo(() => {
    if (owner === "全部BD") {
      return {
        ...
        recharge: 6_346_150,
        yesterday: 287_773,
        completion: 6_346_150 / 11_570_000,
        required: (11_570_000 - 6_346_150) / 17,
      };
    }

    const parentRows = people.filter((item) => item.owner.toLowerCase() === owner.toLowerCase());
    const source = parentRows.length
      ? parentRows
      : agents.filter((item) => item.owner.toLowerCase() === owner.toLowerCase());
    const target = source.reduce((sum, item) => sum + item.target, 0);
    const recharge = source.reduce((sum, item) => sum + item.recharge, 0);
    const yesterday = source.reduce((sum, item) => sum + item.yesterday, 0);
    return { target, recharge, yesterday, completion: target ? recharge / target : 0, required: Math.max(target - recharge, 0) / 17 };
  }, [owner]);
  const attention = [...people].filter((item) => (owner === "全部BD" || item.owner.toLowerCase() === owner.toLowerCase()) && item.recharge / item.target < .433).sort((a, b) => a.recharge / a.target - b.recharge / b.target).slice(0, 4);
  return <main className="dashboard-shell">
    ...
    <div className="workspace">
      ...
      <section className="control-row" aria-label="排名筛选"><div className="filter-group"><label>BD</label><select value={owner} onChange={(event) => setOwner(event.target.value)}><option>全部BD</option><option>Katrina</option><option>Owen</option><option>Luke</option><option>VICTOR</option><option>RUSLAN</option><option>Richard</option><option>Mike</option><option>Patrick</option></select></div><div className="filter-group"><label>排名指标</label><select value={metric} onChange={(event) => setMetric(event.target.value as Metric)}><option>充值金额</option><option>完成率</option><option>昨日充值</option></select></div><div className="search-box"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索 BD、代理商或 API" /></div><p className="updated">更新于 {updated}</p></section>
      <section className="metric-grid" aria-label="总体关键指标"><MetricCard label="月目标" value={formatMoney(overview.target)} hint={owner === "全部BD" ? "2026 年 9 月总目标" : `${owner} 的月目标`} /><MetricCard label="累计充值" value={formatMoney(overview.recharge)} hint={`昨日充值 ${formatMoney(overview.yesterday)}`} /><MetricCard label="整体完成率" value={`${(overview.completion * 100).toFixed(1)}%`} hint="13 / 30 天" accent /><MetricCard label="后续日均需完成" value={formatMoney(overview.required)} hint="距离月末还剩 17 天" /></section>
      <section className="content-grid" id="ranking"><div className="ranking-panel"><div className="panel-header"><div><p className="eyebrow">LEADERBOARD</p><h2>{owner === "全部BD" ? `${view}排名` : `${owner} · ${view}`}</h2></div><div className="view-tabs" role="tablist" aria-label="排名类型">{(["总体", "代理商", "API"] as View[]).map((item) => <button key={item} role="tab" aria-selected={view === item} className={view === item ? "selected" : ""} onClick={() => setView(item)}>{item}</button>)}</div></div><div className="table-wrap"><table><thead><tr><th>排名</th><th>{view === "总体" ? "BD" : view}</th><th>归属 BD</th><th>目标</th><th>累计充值</th><th>完成率</th><th>昨日充值</th><th>开卡</th><th>趋势</th><th>状态</th></tr></thead><tbody>{rows.map((row, index) => { const rate = row.recharge / row.target; const status = progressStatus(rate); return <tr key={`${row.name}-${row.owner}`}><td><span className={index < 3 ? "rank rank-top" : "rank"}>{index + 1}</span></td><td className="name-cell"><span className="avatar">{row.name.slice(0, 1).toUpperCase()}</span><b>{row.name}</b></td><td className="owner-cell">{row.owner}</td><td>{formatMoney(row.target)}</td><td className="money">{formatMoney(row.recharge)}</td><td><div className="rate-cell"><b>{(rate * 100).toFixed(1)}%</b><span><i style={{ width: `${Math.min(rate * 100, 100)}%` }} /></span></div></td><td>{formatMoney(row.yesterday)}</td><td>{formatNum(row.cards)}</td><td>{row.change === 0 ? <span className="neutral">—</span> : <span className={row.change > 0 ? "rise" : "fall"}>{row.change > 0 ? <ArrowUpRight size={15} /> : <ArrowDownRight size={15} />}{Math.abs(row.change)}</span>}</td><td><span className={`status ${status.tone}`}>{status.label}</span></td></tr>; })}</tbody></table></div></div>
