...
import { BarChart3, Download, Gauge, LayoutDashboard, RefreshCw, Search, UsersRound, Waypoints } from "lucide-react";
import dashboardData from "./dashboard-data.json";

type View = "总体" | "代理商" | "API";
type Metric = "充值金额" | "完成率" | "当日充值";
type Language = "zh" | "en";
type NavSection = "dashboard" | "team" | "entities" | "trends";
...
export default function Home() {
  ...
  const chartRows = useMemo(() => [...overall].filter((x) => x.target > 0).sort((a, b) => b.recharge / b.target - a.recharge / a.target), [overall]);
  ...
  const navigate = (section: NavSection) => {
    ...
    if (section === "dashboard") { setView("总体"); window.scrollTo({ top: 0, behavior: "smooth" }); return; }
    if (section === "team") setView("总体");
    if (section === "entities") setView("代理商");
    document.getElementById(section === "trends" ? "trend" : "ranking")?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  return <main className="dashboard-shell"><aside className="side-nav"><div className="brand-mark">UP<span>•</span></div><nav aria-label="Main navigation"><button className={`nav-link ${activeNav === "dashboard" ? "active" : ""}`} onClick={() => navigate("dashboard")}><LayoutDashboard size={18} /><span>{t.dashboard}</span></button><button className={`nav-link ${activeNav === "team" ? "active" : ""}`} onClick={() => navigate("team")}><UsersRound size={18} /><span>{t.team}</span></button><button className={`nav-link ${activeNav === "entities" ? "active" : ""}`} onClick={() => navigate("entities")}><Waypoints size={18} /><span>{t.agentsApi}</span></button><button className={`nav-link ${activeNav === "trends" ? "active" : ""}`} onClick={() => navigate("trends")}><BarChart3 size={18} /><span>{t.trends}</span></button></nav><div className="nav-footer"><span className="live-dot" />{t.imported}</div></aside><div className="workspace">
    <header className="topbar"><div><p className="eyebrow">UP OPERATIONS</p><h1>{t.title}</h1></div><div className="topbar-actions"><button className="language-button" onClick={() => setLanguage(language === "zh" ? "en" : "zh")} aria-label="Switch language">{t.language}</button><button className="refresh-button" onClick={() => setUpdated(new Date().toLocaleString(language === "zh" ? "zh-CN" : "en-GB", { hour12: false }))} aria-label={t.refresh}><RefreshCw size={17} /></button><button className="export-button" onClick={() => window.print()}><Download size={16} />{t.export}</button></div></header>
