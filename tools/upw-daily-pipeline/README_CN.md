...
- `wallet_monthly_targets.csv`：从新配置表读取的月总目标，按公开 BD 和 UPay 平均分配后同步到网站。
...
新代理或总代出现时，只需在 `代理关系及月份目标.xlsx` 增加或修改一行。可填写 `总代UID` 或 `上一级UID`：前者按原始关系表的总代匹配，后者按原始关系表的上一级匹配；两者都匹配时上一级UID优先，以便把已单独配置的子代理从总代中拆分出来。L 列 `BD` 是允许公开展示的 BD 名单，名单外的内部商务与未分配散户都会归入 `UPay`，不展示真实内部商务名。
...
- 完整总额：所有符合上述口径的交易都会计入。内部商务和无法匹配的交易均归入 `UPay`，不会从总额中消失；为 UID 补齐归属后，会自动归入对应 BD / 代理商。

## 目标数据

交易、注册、开卡和消费已经自动计算。月目标不属于后台原始交易数据，程序会读取 `代理关系及月份目标.xlsx` 的 `月份 / 目标` 总目标，并平均分配给 L 列的所有公开 BD 和 `UPay`。例如 6 个公开 BD 时，月目标会分为 7 份。修改该表后，下次双击同步程序即可更新网站。
