...
新代理或总代出现时，只需在 `代理关系及月份目标.xlsx` 增加或修改一行。可填写 `总代UID` 或 `上一级UID`：前者按原始关系表的总代匹配，后者按原始关系表的上一级匹配；两者都匹配时总代优先。L 列 `BD` 是允许公开展示的 BD 名单，名单外的内部商务与未分配散户都会归入 `UPay`，不展示真实内部商务名。
