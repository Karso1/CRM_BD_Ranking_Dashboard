...
def main():
    ...
    for month, (summary_name, daily_name) in MONTHS.items():
        ...
        daily_map, yesterday = {}, {}
        for row in daily.iter_rows(min_row=3, values_only=True):
            owner, category, name = row[:3]
            if not (owner and category and name):
                continue
            category = str(category).upper()
            if category not in {"API", "AGENT"}:
                continue
            key = (str(owner).upper(), str(name).strip())
            daily_map[key] = category
            yesterday[key] = {"recharge": number(row[5] if len(row) > 5 else 0), "cards": number(row[4] if len(row) > 4 else 0)}
        ...
        periods.append({"id": f"2026-{month:02d}", "label": f"2026 年 {month} 月", "start": f"2026-{month:02d}-01", "end": end, "overall": overall, "details": details})
