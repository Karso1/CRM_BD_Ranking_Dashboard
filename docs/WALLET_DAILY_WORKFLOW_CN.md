...
### 2. `UW每日数据.xlsx` 的 `代理商明细 `：归属配置

这是目前唯一需要你偶尔人工维护的表，不需要每天重复填。

当新总代理 UID 出现、BD 更换、代理商归属更换时，新增或修正这一行：

```text
总代UID | 商务（BD） | 代理商
```

程序借此把后台用户归属到正确的 BD 与代理商。不要再逐个在“总表 UW 代理下注册用户”中手工写每个用户的归属。
...
- `outputs/history/unmapped_master_uids.csv`：发现新 UID 未归属时查看它，并把需要的归属补到 `代理商明细 `。
...
只有在出现新总代 UID 或归属变化时，打开 `UW每日数据.xlsx`，在 `代理商明细 ` 中维护 `总代UID / 商务 / 代理商`。
